...
| [Жесты трекпада и Safari](docs/ru/trackpad-and-safari.md) | Тап четырьмя пальцами как средний клик и что Safari умеет и не умеет с вкладками |
| [cloudsync](https://github.com/MAXZVER/cloudsync) ↗ | Выборочная синхронизация облака с трёхсторонним слиянием конфликтов, macOS и Windows. Писалось здесь, вынесено отдельно — к XPS не привязано ничем |
